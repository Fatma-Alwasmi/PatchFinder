...
for cve in data.keys():
    if 'fixes' in data[cve] and 'nvd_text' in data[cve] and 'cmt_msg' in data[cve]:
        if data[cve]['fixes'] != "" and data[cve]['nvd_text'] != "":
            hash = data[cve]['fixes']
            description = data[cve]['nvd_text'] 
            description2 = data[cve]['cmt_msg']
            if keyword.lower() in description.lower() or keyword.lower() in description2.lower():
                try:
